export default function ASIHomepageUIUX() {
  ...
  const decisionFeed = [
    ...
    { id: "DEC-1038", title: "Ops Schedule Adjustment", owner: "Operations AI", state: "Approved", risk: "Low" },
  ];
  ...
  const chatPositions = [
    ...
    { area: "Settings Assistant", detail: "ในหน้าตั้งค่าเพื่อช่วย configure system และอธิบายผลกระทบ" },
  ];

  return (
    <div className="min-h-screen bg-slate-950 text-slate-100">
      <div className="flex min-h-screen">
        <aside className="hidden lg:flex w-72 flex-col border-r border-white/10 bg-slate-900/80 backdrop-blur-xl">
          ...
          <header className="sticky top-0 z-20 border-b border-white/10 bg-slate-950/80 backdrop-blur-xl">
            <div className="mx-auto flex max-w-7xl items-center justify-between gap-4 px-4 py-4 lg:px-8">
              ...
              <div className="flex items-center gap-3">
                <button className="rounded-2xl border border-white/10 px-4 py-2 text-sm text-slate-200 hover:bg-white/5">Command Palette</button>
                <button className="rounded-2xl bg-cyan-400 px-4 py-2 text-sm font-semibold text-slate-950 hover:opacity-90">New Workflow</button>
              </div>
            </div>
          </header>
          ...
          <div className="mx-auto max-w-7xl space-y-8 px-4 py-8 lg:px-8">
              ...
              <div className="rounded-[28px] border border-white/10 bg-gradient-to-br from-slate-900 to-slate-950 p-6 shadow-2xl">
                <div className="max-w-3xl">
                  ...
                  <p className="mt-4 max-w-2xl text-sm leading-7 text-slate-300 lg:text-base">
                    ออกแบบให้เป็นจุดเริ่มต้นของบริการทั้งหมด: เริ่มงานใหม่, ดูการตัดสินใจ, ตรวจ governance, ตรวจสอบ lineage,
                    และสนทนากับ AI ได้ทั้งในระดับ global, section และ domain-specific
                  </p>
                </div>
              ...
              <div className="rounded-[28px] border border-white/10 bg-slate-900/90 p-6 shadow-2xl">
                ...
                <div className="mt-5 space-y-3 rounded-[24px] border border-white/10 bg-slate-950/70 p-4">
                  ...
                  <div className="max-w-[92%] rounded-2xl border border-white/10 bg-white/5 px-4 py-3 text-sm leading-6 text-slate-200">
                    พบ 3 รายการ: Budget Reallocation (Finance AI), Vendor Risk Flag (Risk AI), Policy Override Request (Governance).
                    คุณสามารถเปิด panel รายการเหล่านี้ด้านล่างหรือสั่งให้ฉันกรองเฉพาะรายการที่ต้องการ CFO approval ได้
                  </div>
                </div>

                <div className="mt-4 flex gap-2 overflow-x-auto pb-1">
                  {["/start workflow", "/pending approvals", "/drift alerts", "/open lineage"].map((cmd) => (
                    <button key={cmd} className="whitespace-nowrap rounded-full border border-white/10 px-3 py-2 text-xs text-slate-300 hover:bg-white/5">
                      {cmd}
                    </button>
                  ))}
                </div>
            ...
            <section className="grid gap-6 lg:grid-cols-[1.15fr_0.85fr]">
              <div className="rounded-[28px] border border-white/10 bg-slate-900/80 p-6">
                <div className="flex items-center justify-between">
                  ...
                  <button className="rounded-2xl border border-white/10 px-4 py-2 text-sm text-slate-200 hover:bg-white/5">View All Departments</button>
                </div>
                  ...
                  {deptCards.map((dept) => (
                    <div key={dept.name} className="rounded-[24px] border border-white/10 bg-white/5 p-5 shadow-lg">
                      <div className="flex items-start justify-between gap-4">
                        ...
                        <span className="rounded-full border border-white/10 bg-black/20 px-3 py-1 text-xs text-slate-300">{dept.status}</span>
                      </div>
                      <div className="mt-5 flex gap-2">
                        <button className="rounded-xl bg-white px-3 py-2 text-xs font-medium text-slate-900">{dept.ai}</button>
                        <button className="rounded-xl border border-white/10 px-3 py-2 text-xs text-slate-200">Open Dashboard</button>
                        <button className="rounded-xl border border-white/10 px-3 py-2 text-xs text-slate-200">Policies</button>
                      </div>
                    </div>
                  ))}
                </div>
              ...
              <div className="rounded-[28px] border border-white/10 bg-slate-900/80 p-6">
                ...
                <p className="mt-2 text-sm leading-6 text-slate-400">
                  ผู้ช่วยเฉพาะส่วน ใช้ถามคำถามเชิงแผนก เช่น “Finance AI มีงานไหนติด approval”, “Legal AI review policy ไหนค้างอยู่”
                </p>
                <div className="mt-5 space-y-3 rounded-[24px] border border-white/10 bg-slate-950/70 p-4">
                  <div className="rounded-2xl bg-white/5 px-4 py-3 text-sm text-slate-200">Finance AI ตอนนี้มี 2 รายการที่เกิน threshold และ 1 รายการที่ต้อง CFO review</div>
                  <div className="ml-auto rounded-2xl bg-cyan-400 px-4 py-3 text-sm text-slate-950">เปิดรายการที่ต้อง CFO review ให้หน่อย</div>
                </div>
                <input className="mt-4 w-full rounded-2xl border border-white/10 bg-slate-950 px-4 py-3 text-sm placeholder:text-slate-500" placeholder="ถามผู้ช่วยประจำ section นี้..." />
              </div>
            </section>
            ...
            <section className="grid gap-6 xl:grid-cols-[1.2fr_0.8fr]">
              <div className="rounded-[28px] border border-white/10 bg-slate-900/80 p-6">
                <div className="flex items-center justify-between">
                  ...
                  <button className="rounded-2xl border border-white/10 px-4 py-2 text-sm text-slate-200 hover:bg-white/5">Open Decision Board</button>
                </div>
                    ...
                    <div key={item.id} className="rounded-[24px] border border-white/10 bg-white/5 p-5">
                      <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
                        <div>
                          ...
                          <div className="mt-2 text-sm text-slate-400">Owner: {item.owner} · State: {item.state} · Risk: {item.risk}</div>
                        </div>
                        <div className="flex flex-wrap gap-2">
                          <button className="rounded-xl bg-white px-3 py-2 text-xs font-medium text-slate-900">Open</button>
                          <button className="rounded-xl border border-white/10 px-3 py-2 text-xs text-slate-200">Explain with AI</button>
                          <button className="rounded-xl border border-white/10 px-3 py-2 text-xs text-slate-200">View Lineage</button>
                        </div>
                      </div>
                    </div>
                  ))}
              ...
              <div className="rounded-[28px] border border-white/10 bg-slate-900/80 p-6">
                ...
                <p className="mt-2 text-sm leading-6 text-slate-400">
                  แชทนี้ฝังอยู่ใน decision detail panel ทุกใบ ใช้ถามเหตุผล, policy ที่เกี่ยวข้อง, confidence, risk, และ lineage proof
                </p>
                <div className="mt-5 rounded-[24px] border border-white/10 bg-slate-950/70 p-4">
                  <div className="rounded-2xl bg-white/5 px-4 py-3 text-sm text-slate-200">
                    DEC-1042 ถูกตั้งเป็น Awaiting CFO Approval เพราะ amount_delta เกิน policy threshold POL-FIN-001 และ Risk AI ให้ระดับ Medium
                  </div>
                </div>
                <input className="mt-4 w-full rounded-2xl border border-white/10 bg-slate-950 px-4 py-3 text-sm placeholder:text-slate-500" placeholder="ถามเกี่ยวกับ decision นี้..." />
              </div>
            </section>
            ...
            <section className="grid gap-6 xl:grid-cols-[1fr_1fr]">
              <div className="rounded-[28px] border border-white/10 bg-slate-900/80 p-6">
                ...
                <div className="mt-5 rounded-[24px] border border-white/10 bg-slate-950/70 p-5 text-sm leading-7 text-slate-300">
                  ทุก decision material ต้องเปิด lineage ได้ด้วยปุ่มเดียว พร้อม AI helper สำหรับอธิบาย event chain แบบภาษาธรรมดาให้ผู้บริหารเข้าใจง่าย
                </div>
                ...
                <div className="mt-5 grid gap-4 sm:grid-cols-2">
                  {settingGroups.map((group) => (
                    <div key={group.title} className="rounded-[24px] border border-white/10 bg-white/5 p-5">
                      ...
                      <button className="mt-4 rounded-xl border border-white/10 px-3 py-2 text-xs text-slate-200">Configure</button>
                    </div>
                  ))}
                </div>
              </div>
            ...
            <section className="rounded-[28px] border border-white/10 bg-slate-900/80 p-6">
              <div className="flex flex-col gap-2 lg:flex-row lg:items-end lg:justify-between">
                ...
                <button className="rounded-2xl border border-white/10 px-4 py-2 text-sm text-slate-200 hover:bg-white/5">Open Full Chat Architecture</button>
              </div>
